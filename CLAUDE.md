...
- **outputease session workflow** — the fork's recommended stick semantics, the `@outputease/toolkit` session rituals (`/quickstart`, `/dev-check`, `/checkpoint`, `/session-end`), and the verified Claude Code 2.1.x key map (byte-level, with the Ctrl+O/Ctrl+R and Tab/Alt+T drift from older releases) are documented in [docs/outputease.md](docs/outputease.md). Consult it before adding or changing any Claude Code key binding; re-verify shortcuts against the live CLI on version bumps.
