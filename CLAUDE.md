...
- **Platforms: Windows and Linux.** Upstream is macOS-first; outputease runs Windows and Linux, so macOS-only paths are candidates for porting or clean degradation. Known macOS-only pieces: the `codex-app` harness (drives the Codex desktop app via `osascript`/`codex://` deep links), the `postinstall` chmod of node-pty's spawn-helper (POSIX-only, harmless elsewhere), and the `ioreg` troubleshooting tip in the README. CI currently runs ubuntu + macos only — no Windows job yet.
- **Primary controller: 8BitDo Pro 2 Ultimate** — not explicitly supported yet (no driver routing or fixture). 8BitDo pads present differently per mode switch (XInput mode may hit the xbox driver; other modes fall to the best-effort generic parser). Adding it follows the standard flow in CONTROLLERS.md: `openmicro doctor` → `npm run capture` → parser in `src/controller/` routed by VID/PID → fixture in `test/fixtures/controllers/`. Like every non-DualSense pad it will be input-only (no lightbar/LED feedback).
