...
mom claude # mom (MoreOpenMicro) is an alias for openmicro
```
...
OpenMicro installs its lifecycle hooks automatically. If Codex reports that its hooks changed, open `/hooks` in Codex and trust the OpenMicro hooks. Codex only runs hooks when its `plugin_hooks` feature is enabled — add `plugin_hooks = true` under `[features]` in `~/.codex/config.toml` (verified against codex-cli 0.144; without it, hooks are silently skipped and OpenMicro gets no Codex state feedback). On Windows the installed entries carry a `commandWindows` variant pointing at an installed PowerShell wrapper (`~/.codex/openmicro-hook.ps1`), because Codex's Windows hook runner executes command strings without any shell.
